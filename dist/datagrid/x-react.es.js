...
function ee({
  ...
  var f;
  const [x, v] = C.useState(null), j = C.useRef(a), [R, p] = C.useState({
    inView: !!r,
    entry: void 0
  });
  j.current = a, C.useEffect(
    () => {
      if (s || !x) return;
      let y;
      return y = P(
  ...
  const S = (f = R.entry) == null ? void 0 : f.target, k = C.useRef(void 0);
  !x && S && !u && !s && k.current !== S && (k.current = S, p({
    inView: !!r,
    entry: void 0
  }));
